...
function Page() {
  ...
  return (
      <div className=" bg-[#40534C] flex flex-col grow rounded-xl overflow-hidden w-3/4 mx-auto mb-4">
        <header className="text-xl text-center bg-[#1A3636] p-1">Rejected Problems</header>
        ...
        <div className="grow overflow-y-auto">
          <div className="h-full flex flex-col">
            {problemsData?.map((problem: any, index: number) => (
              <div className="flex mx-4 text-sm gap-2 border-b-[1px] py-2 border-[#677D6A] justify-center items-center" key={problem.title}>
                ...
                {/* <Link
                  key={problem?._id}
                  href={`moderator/verification/${problem.title.split(' ').join('-').toLowerCase()}`}
                  className="w-[15%] p-1 flex justify-center items-center"
                >
                  <button className="bg-[#1A3636] p-1 px-4 rounded-xl text-white hover:font-bold">View</button>
                </Link> */}
              </div>
            ))}
          </div>
